...
// https://etherscan.io/address/0x99120d128a5f7cb81c318a24fa1f60f66d9777d7#code
// https://rinkeby.etherscan.io/address/0x7551dc208fdb308c52a27cae25e9ba8e76ef2733#contracts
contract PPADealers is ERC721A, Ownable {
    ...
    uint256 public publicPriceWei;//公开价格
    ...
    // WL=white list 白名单，通过签名来验证购买者是否在白名单中
    ...
    //测试solidity中的keccak256()
    function encodeParams(
        address addr,
        uint256 totalMintsAllowed,
        uint256 targetSaleState
    ) public  pure returns(bytes memory,bytes32,bytes32) {
        bytes memory encode =  abi.encodePacked(addr, totalMintsAllowed, targetSaleState);
        bytes32 inputHash = keccak256(encode);
        bytes32 ethSignedMessageHash = inputHash.toEthSignedMessageHash();
        return(encode,inputHash,ethSignedMessageHash);
    }
